use std::{collections::HashSet, fs, iter::Sum};
use regex::Regex;
...
fn parse_input(split_input: &Vec<String>) -> (&Vec<String>, Vec<String>, Vec<String>, Vec<String>) {
    let char_grid: Vec<Vec<char>> = split_input.iter().map(|line| line.chars().collect()).collect();
...
fn part_1(horizontal: &Vec<String>, vertical: &Vec<String>, rdiag: &Vec<String>, ldiag: &Vec<String>) -> usize { 
    let xmas_re = Regex::new(r"(XMAS)").unwrap();
    let samx_re  = Regex::new(r"(SAMX)").unwrap();
    let horizontal_matches: usize = horizontal.iter().map(|s| xmas_re.find_iter(s).count() + samx_re.find_iter(s).count()).sum();
    let vertical_matches: usize = vertical.iter().map(|s| xmas_re.find_iter(s).count() + samx_re.find_iter(s).count()).sum();
    let rdiag_matches: usize = rdiag.iter().map(|s| xmas_re.find_iter(s).count() + samx_re.find_iter(s).count()).sum();
    let ldiag_matches: usize = ldiag.iter().map(|s| xmas_re.find_iter(s).count() + samx_re.find_iter(s).count()).sum();
...
fn is_valid(char_grid: &Vec<Vec<char>>, loc: &(usize, usize)) -> bool {
    let diag1 = HashSet::from([char_grid[loc.1 + 1][loc.0 + 1], char_grid[loc.1 - 1][loc.0 - 1]]);
    let diag2 = HashSet::from([char_grid[loc.1 + 1][loc.0 - 1], char_grid[loc.1 - 1][loc.0 + 1]]);
...
fn part_2(split_input: &Vec<String>) -> usize {
    ...
    let a_locs: Vec<Vec<(usize, usize)>> = split_input.iter().enumerate().map(|(y, s)| a_regex.find_iter(s).map(|m| (m.start(), y)).collect()).collect();
    let x_max = split_input[0].len();
    let y_max = split_input.len();

    let interior_a: Vec<&(usize, usize)> = a_locs.iter().flatten().filter(|(x, y)| x > &0 && y > &0 && x < &(x_max - 1) && y < &(y_max- 1)).collect();
    // Avoids going out of bounds when checking diagonal

    return interior_a.iter().filter(|x| is_valid(&char_grid, x)).count();
...
fn main() {
    ...
    println!("Part 1: {}", part_1(&split_string, &vertical, &rdiag, &ldiag));
